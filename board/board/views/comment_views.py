...
def comment_create_answer(request,aid):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            #  name 앵커 이용
            return redirect("{}#comment_{}".format(resolve_url("board:detail", qid=answer.question.id), comment.id))
    else:
        form = CommentForm()
    return render(request, "board/comment_form.html",{"form":form})
...
def comment_edit_answer(request,comment_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect("{}#comment_{}".format(resolve_url("board:detail", qid=comment.answer.question.id), comment.id))
    else:
        form = CommentForm(instance=comment)
    return render(request, "board/comment_form.html", {"form":form})
